...
def top_articles():
    ...
    title = '*** TOP ARTICLES ***'
    print('\n' + txt.CYAN + txt.BOLD + txt.UNDERLINE
          + title.center(55) + txt.END + '\n')
    for r in result:
...
def top_authors():
    ...
    title = '*** TOP AUTHORS ***'
    print('\n' + txt.GREEN + txt.BOLD + txt.UNDERLINE
          + title.center(55) + txt.END + '\n')
    for r in result:
        print(txt.BOLD + "  ", r[0], ":  " + txt.END, r[1], "views")
    db.close()
...
def error_log():
    ...
    query = """
                select TO_CHAR(bad_req.dt,'Mon DD, YYYY') as date,
                    CONCAT(CAST (ROUND(bad_req.num::decimal*100/all_req.num,2)
                    as VARCHAR)) as percent
    ...
    print('\n' + txt.RED + txt.BOLD + txt.UNDERLINE
          + title.center(55) + txt.END + '\n')
    for r in result:
